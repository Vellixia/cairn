...
| 9 | **Local model download is unverified.** `fastembed`/`hf-hub` downloads `all-MiniLM-L6-v2` with no checksum. | `crates/cairn-embed/src/lib.rs:254-270` | Pin known SHA-256 of the artifact and verify before loading. | **Open** |
...
| 12 | **`embed-local` default in Docker is heavy and slow.** Pulls `fastembed`/`ort`/`aws-lc-sys`. | `Dockerfile` `ARG CAIRN_FEATURES=embed-local` | Default Docker image to lean build; document opt-in for local embeddings. | **Open** — documented but still default |
