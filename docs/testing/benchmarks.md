...
3. **Smart memory retention** - does Cairn's `confidence x pin x crystallize` policy
...
- **Cairn policy** - drop the lowest `confidence x importance` non-pinned memory.
...
|---|---|---|
| Naive LRU | ~6/20 | ~30% |
| **Cairn (confidence x importance + pin)** | **~14/20** | **~70%** |

Cairn preserves roughly **2x more important memories** than LRU after 50 cycles
...
- **70% retention vs 30% LRU** is the headline result for the retention
  benchmark - it directly motivates `confidence x importance x pin` as the
  default policy.
...
- [Architecture](../reference/architecture.md) - how the assemble + retention pipeline works internally
- [Roadmap](../planning/roadmap.md) - benchmark implementation status (Sprint 16 marked done)
- [Plan](../archive/plan-v0.5.0.md) - benchmark targets (Sprint 16) and CI plan
- [ADR-023](../reference/decisions.md) - why we hand-build fixtures instead of redistributing upstream data
- [ADR-024](../reference/decisions.md) - landing-page architecture (Sprint 17)
