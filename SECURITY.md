...
| **Compromised endpoint** — attacker has the user's device | Argon2id can't help here; user-level controls (screen lock, device encryption) are the right layer. Out of scope. | Documented |
...
- **Argon2id** for passphrase → key derivation (Sprint 15b). Parameters: 64 MiB
...
- **HMAC-SHA256** for the cost-savings ledger (Sprint 5). The same
  `CAIRN_SECRET_KEY` that signs JWT device tokens signs the ledger — both are
...
1. Set `CAIRN_SECRET_KEY` to a ≥32-byte random value (`openssl rand -base64 48`).
...
4. Enable E2E encryption for sync (`cairn-cli sync --e2e --passphrase …`).
